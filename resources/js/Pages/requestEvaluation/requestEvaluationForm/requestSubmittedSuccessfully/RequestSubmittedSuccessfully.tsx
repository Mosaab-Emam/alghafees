...
const RequestSubmittedSuccessfully = ({
    ...
    const handleCopyToClipboard = async () => {
        try {
            if (request_no) await navigator.clipboard.writeText(request_no);
            setCopied(true);
            ...
            <div className=" flex flex-col justify-center items-center self-stretch gap-2">
                ...
                <div className="flex flex-col items-center self-stretch gap-8">
                    <p className="regular-b1 text-center text-Gray-scale-02">
                        شكرًا لك على تقديم طلب تقييم العقار! نحن نعمل الآن على
                        معالجة طلبك وسنتواصل معك قريبًا للحصول على التفاصيل
                        اللازمة. إذا كان لديك أي استفسارات، لا تتردد في الاتصال
                        بنا.
                    </p>
