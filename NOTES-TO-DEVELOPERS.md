...
{
// If $state is already a URL or a data URI, return it directly
if (filter_var($state, FILTER_VALIDATE_URL) !== false || str($state)->startsWith('data:')) {
return $state;
}

        // If shouldCheckFileExistence is enabled
        if ($this->shouldCheckFileExistence()) {
            // Check if the file exists in the public folder
            if (file_exists(public_path($state))) {
                return asset($state); // Return the public URL
            }

            // Check if the file exists in the storage folder
            $storage = $this->getDisk();
            if ($storage->exists($state)) {
                // If visibility is private, generate a temporary URL
                ...
                // Otherwise, return the URL generated by the storage disk
                return $storage->url($state);
            }
        }

        // File does not exist in either public or storage folder
        return null;
    }
```
